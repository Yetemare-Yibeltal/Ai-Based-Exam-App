...
const { generateTokenPair, getTokenExpiry, verifyRefreshToken } = require('../utils/generateToken');
const { generateEmailVerificationOTP, generatePasswordResetOTP, hashOTP, verifyOTP, isOTPExpired } = require('../utils/otpGenerator');
const { sendWelcomeEmail, sendEmailVerificationOTP, sendPasswordResetOTP, sendPasswordChangedEmail } = require('../utils/sendEmail');
...
const rotateRefreshToken = async (oldRefreshToken, userId, userModel, role, req) => {
  const { valid, expired, decoded } = verifyRefreshToken(oldRefreshToken);

  if (!valid) {
    throw new Error(expired ? 'Session expired. Please log in again' : 'Invalid refresh token');
  }
...
const loginUser = async (email, password, role, req) => {
  ...
  const modelName = role === 'teacher' ? 'Teacher' : role === 'admin' ? 'Admin' : 'User';
  ...
  if (user.isBanned) throw new Error(`Your account has been banned. Reason: ${user.banReason || 'Violation of terms'}`);
...
const verifyUserEmail = async (email, otp, role) => {
  ...
  if (!user.emailVerificationOTP) throw new Error('No verification OTP found. Please request a new one');
  if (isOTPExpired(user.emailVerificationOTPExpiry)) throw new Error('OTP has expired. Please request a new one');
...
const resetUserPassword = async (email, otp, newPassword, role) => {
  ...
  if (!user) throw new Error('No account found with this email');
  if (!user.passwordResetOTP) throw new Error('No reset request found. Please request a new OTP');
  if (isOTPExpired(user.passwordResetOTPExpiry)) throw new Error('OTP has expired. Please request a new one');

  const isValid = verifyOTP(otp, user.passwordResetOTP);
  if (!isValid) throw new Error('Invalid OTP. Please check and try again');

  const isSame = await user.comparePassword(newPassword);
  if (isSame) throw new Error('New password cannot be the same as current password');
...
const changeUserPassword = async (userId, currentPassword, newPassword, role) => {
  const Model = getModelByRole(role);
...
const uploadUserAvatar = async (userId, file, role, folder) => {
  ...
  if (!uploadResult.success) throw new Error('Image upload failed. Please try again');
