...
exports.approveQuestion = catchAsync(async (req, res) => {
  ...
  if (admin) {
    ...
    await admin.logActivity(
      ...
      "Question",
      question._id,
      `Approved ${question.subject} question`,
      req.ip,
    );
  }
...
exports.rejectQuestion = catchAsync(async (req, res) => {
  ...
  logger.info(
    `Question rejected — Question: ${question._id} — Admin: ${req.userId}`,
  );
...
exports.getPendingQuestions = catchAsync(async (req, res) => {
  const { page, limit, skip } = getPagination(req.query);
  const { subject, isAIGenerated } = req.query;
...
exports.bulkApprove = catchAsync(async (req, res) => {
  ...
  if (questions.length === 0)
    return errorResponse(res, "No pending questions found", 404);
...
exports.bulkReject = catchAsync(async (req, res) => {
  ...
  if (admin)
    await admin.logActivity(
      ...
      "Question",
      null,
      `Bulk rejected ${rejectedCount} questions`,
      req.ip,
    );
